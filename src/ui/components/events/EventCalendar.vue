...
function fillEvents(): void {
    for (let i = 0; i < props.events.length; i++) {
        ...
        const overlapsWithPrevious = eventService.doEventsHaveOverlappingDays(props.events[i - 1], props.events[i]);
        const overlapsWithNext = eventService.doEventsHaveOverlappingDays(props.events[i], props.events[i + 1]);
