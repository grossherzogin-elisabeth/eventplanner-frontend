...
import type { Position, User } from '@/app/types';
import { VInputDate, VInputLabel, VInputSelect, VInputText, VTable } from '@/lib/components';
...
async function fetchUserEvents(): Promise<void> {
    ...
    const evts = await ctx.events.getEventsByUser(year, userKey);
    const positions = await ctx.users.resolvePositionNames();
    events.value = evts
        .map((evt) => {
