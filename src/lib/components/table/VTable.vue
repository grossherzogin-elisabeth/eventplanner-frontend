                ...
                <tr class="no-data">
                    <slot name="no-data" :colspan="columnCount">
                        <td :colspan="columnCount" class="italic">
                            Keine Daten gefunden. Versuchen Sie einen anderen Filter, oder legen Sie einen neuen
                            Datensatz an.
                        </td>
                    </slot>
                </tr>
...
import { ObjectUtils, RouterUtils } from '@/lib/utils';
...
function updateQuery(prop: string, value: number | string): void {
    if (props.query) {
        RouterUtils.updateQuery(router, prop, value);
    }
}
